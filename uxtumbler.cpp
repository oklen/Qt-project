...
void UXTUMBLER::mouseMoveEvent(QMouseEvent *event)
{
    if(curval != (stayval -  0.02*(event->pos() - initpoint).x()))
    {
        ...
        update(); // Used to call the paintevent (Qt paint system function!)
    }
    if(curval>=maxbound)
    {
        curval=maxbound;
...
void UXTUMBLER::paintEvent(QPaintEvent *event)
{
    ...
    mpainter->drawLine(-10,5,300,5);
    mpainter->drawLine(-10,43,300,43);
...
//    }
    ...
    for(int i=0;i<6;i++)
    {
        mpainter->drawText(QRect(int((i-repair)*50)+b,15,30,35),QString::number(int(leftv)+i));
    }
    mpen.setWidth(3);
    mpainter->setPen(mpen);
    mpainter->drawLine(157,45,157,35);
    mpainter->end();
}
...
void UXTUMBLER::setmaxvalue(qreal max)
{
    maxbound = max;
}

void UXTUMBLER::setminvalue(qreal min)
{
    minbound = min;
}
