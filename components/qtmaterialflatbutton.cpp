...
void QtMaterialFlatButton::setborder(bool border)
{
    Q_D(QtMaterialFlatButton);
...
void QtMaterialFlatButton::paintForeground(QPainter *painter)
{
    ...
    QRect iconGeometry(pos + QPoint((width()-iconSize().width())/2, (height()-iconSize().height())/2), iconSize());
...
//    icon.fillRect(pixmap.rect(), painter->pen().color());
    painter->drawPixmap(QRect(QPoint((width()-iconSize().width())/2, (height()-iconSize().height())/2), iconSize()), pixmap);
}
...
void QtMaterialFlatButton::invokeriddle()
{
    Q_D(QtMaterialFlatButton);

    QPoint pos;
    pos = QPoint(-100,rect().center().y());

    QtMaterialRipple *ripple = new QtMaterialRipple(pos);

    ripple->setRadiusEndValue(500);
    ripple->setOpacityStartValue(0.9);
    ripple->setColor(QColor("#7bcb5e"));
    ripple->radiusAnimation()->setDuration(1200);
    ripple->opacityAnimation()->setDuration(3000);
